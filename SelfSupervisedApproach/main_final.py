...
from DepthNet import DepthNet18, DepthNet50,DepthNetbase





def compute_mean_depth_loss(depth: torch.Tensor, target_mean: float = 10.0, weight: float = 0.1) -> torch.Tensor:
    return weight * torch.abs(depth.mean() - target_mean) / target_mean
...
def compute_photometric_loss(target_image, reconstructed_image):
    ...
    alpha = 0.85  # Adjustable
    photometric_loss = alpha * ssim_loss.mean(1, True) + (1 - alpha) * l1_loss.mean(1, True)
...
def save_visualization_improved(curr_image, depth, epoch, batch_idx,experiment_idx):
    ...
    #cv2.imwrite(f'VisualizedResults/depth_vis_epoch_{epoch}_batch_{batch_idx}_exp_{experiment_idx}.png', combined_img)
    # Save depth values in case we need them for further analysis
    np.save(f'VisualizedResults/depth_raw_epoch_{epoch}_batch_{batch_idx}_exp_{experiment_idx}.npy', depth_np)
...
def compute_adaptive_mean_depth_loss(depth, weight=0.1):
    # Calculate the mean of the current batch
    batch_mean = depth.mean()
    
    # Use the batch mean as the target mean
    target_mean = batch_mean.detach()
    
    # Compute the loss
    mean_loss = weight * torch.abs(batch_mean - target_mean) / (target_mean + 1e-7)
    
    return mean_loss

def get_experiment_config(i):
    if i == 0:
        name = "Baseline Model"
        target_mean_loss_weight = 1.0
        depth_net = DepthNetbase()
        ...
        name = "Reduced Target Mean Loss"
        target_mean_loss_weight = 0.1
        depth_net = DepthNet18()  
        KITTI_Raw_Data_set = KITTIRawDataset
        pose_net= PoseNet(num_input_images=2)
        weight_decay = 1e-5
        ...
        target_mean_loss_weight = 1.0
        depth_net = DepthNet18()  
        KITTI_Raw_Data_set = KITTIRawDatasetWithAugmentation
        pose_net= PoseNet(num_input_images=2)
        weight_decay = 1e-5
        ...
        target_mean_loss_weight = 1.0
        depth_net = DepthNet50()  # Use ResNet50
        KITTI_Raw_Data_set = KITTIRawDataset
        pose_net=PoseNet(num_input_images=2)
        weight_decay = 1e-5
        

    return depth_net, target_mean_loss_weight,KITTI_Raw_Data_set,pose_net,weight_decay,name
...
def main():
    ...
    for i in range(7):  # Adjust the range based on the number of experiments
        ...
        val_dataset = KITTI_Raw_Data_set(root_dir=root_dir, sequences=val_sequences, start=0, end=5)
        ...
        optimizer = torch.optim.Adam(depth_net.parameters(), lr=1e-3, weight_decay=weight_decay if i == 5 else 0)
